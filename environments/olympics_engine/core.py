...
from environments.olympics_engine.viewer import Viewer, debug
from environments.olympics_engine.tools.func import *
from environments.olympics_engine.tools.settings import *
...
class OlympicsBase(object):
    ...
    def check_valid_map(
        self,
    ):  # not using due to conflicting with arc center repitition...
        object_init_list = [
            str(self.map["objects"][i].init_pos)
            for i in range(len(self.map["objects"]))
        ]
        temp = self.map["objects"]
        print(
            "len1 = {}, len2 = {}".format(
                len(object_init_list), len(set(object_init_list))
            )
        )
    ...
    def init_state(self):
        ...
        for i in range(len(self.agent_list)):
            ...
            init_obs = (
                self.view_setting["init_obs"][i]
                if "init_obs" in self.view_setting
                else 0
            )  # random_theta
        ...
        for agent_idx in range(self.agent_num):
            ...
            for object_idx in range(len(self.map["objects"])):
                if self.map["objects"][object_idx].type == "wall":
                    ...
                    if distance < r:
                        raise ValueError(
                            "The position of agent {} overlap with object {}. Please reinitialise...".format(
                                agent_idx, object_idx
                            )
                        )
            for rest_idx in range(agent_idx + 1, self.agent_num):
                ...
                if distance < (r1 + r2) ** 2:
                    raise ValueError(
                        "The position of agent {} overlap with agent {}.".format(
                            agent_idx, rest_idx
                        )
                    )
    ...
    def _circle_collision_response(self, coord1, coord2, v1, v2, m1, m2):
        ...
        return [u1_x * self.circle_restitution, u1_y * self.circle_restitution], [
            u2_x * self.circle_restitution,
            u2_y * self.circle_restitution,
        ]

    def CCD_circle_collision(
        self, old_pos1, old_pos2, old_v1, old_v2, r1, r2, m1, m2, return_t
    ):
        """
        ...
        l = (relative_pos[0] ** 2 + relative_pos[1] ** 2 - (r1 + r2) ** 2) / (
            relative_v[0] ** 2 + relative_v[1] ** 2
        )
        ...
        # handle collision
        v1_col, v2_col = self._circle_collision_response(
            pos_col1, pos_col2, old_v1, old_v2, m1, m2
        )  # the position and v at the collision time

        return pos_col1, v1_col, pos_col2, v2_col

    def bounceable_wall_collision_time(
        self, pos_container, v_container, remaining_t, ignore
    ):
        col_target = None
        col_target_idx = None
        current_idx = None
        ...
        for agent_idx in range(self.agent_num):
            ...
            for object_idx in range(len(self.map["objects"])):
                ...
                if object.can_pass():  # cross
                    continue
                if (
                    object.ball_can_pass and self.agent_list[agent_idx].type == "ball"
                ):  # for table hockey game
                    continue

                # check the collision time and the collision target (wall and endpoint collision)
                temp_t, temp_col_target = object.collision_time(
                    pos=pos, v=v, radius=r, add_info=[agent_idx, object_idx, ignore]
                )

                if (
                    abs(temp_t) < 1e-10
                ):  # the collision time computation has numerical error
                    temp_t = 0
                ...
                if 0 <= temp_t < current_min_t:
                    if temp_col_target == "wall" or temp_col_target == "arc":
                        ...
                        check = [
                            agent_idx,
                            getattr(object, temp_col_target),
                            temp_t,
                        ] not in ignore
                    else:
                        raise NotImplementedError(
                            "bounceable_wall_collision_time error"
                        )
    ...
    def wall_response(self, target_idx, col_target, pos, v, r, t):
        ...
        col_pos, col_v = object.collision_response(
            pos, v, r, col_target, t, self.wall_restitution
        )

        return col_pos, col_v

    def update_other(
        self, pos_container, v_container, t, already_updated
    ):  # should we also contain a???
        for agent_idx in range(self.agent_num):
    ...
    def circle_collision_time(
        self, pos_container, v_container, remaining_t, ignore
    ):  # ignore = [[current_idx, target_idx, collision time]]
        ...
        for agent_idx in range(self.agent_num):
            ...
            for rest_idx in range(agent_idx + 1, self.agent_num):
                ...
                collision_t = self.CCD_circle_collision(
                    pos1, pos2, v1, v2, r1, r2, m1, m2, return_t=True
                )
                ...
                if (
                    0 <= collision_t < current_min_t
                ):  # and [agent_idx, rest_idx, collision_t] not in ignore:
    ...
    def actions_to_accel(self, actions_list):
        ...
        for agent_idx in range(self.agent_num):
            ...
            if action is None:
                ...
                if self.agent_list[
                    agent_idx
                ].is_fatigue:  # if agent is out of energy, no driving force applies
                    accel = [0, 0]
                else:
                    mass = self.agent_list[agent_idx].mass

                    assert self.action_f[0] <= action[0] <= self.action_f[1], print(
                        "Continuous driving force needs "
                        "to be within the range [-100,200]: {0}".format(action[0])
                    )
                    force = action[0] / mass

                    assert (
                        self.action_theta[0] <= action[1] <= self.action_theta[1]
                    ), print(
    ...
    def _add_wall_ignore(
        self,
        collision_wall_target,
        current_agent_idx,
        target_wall_idx,
        ignore_wall,
        if_global=False,
            ...
            if if_global:
                self.global_wall_ignore.append(
                    [
                        current_agent_idx,
                        getattr(
                            self.map["objects"][target_wall_idx], collision_wall_target
                        ),
                        0,
                    ]
                )
    ...
    def handle_wall(
        ...
        col_pos, col_v = self.wall_response(
            target_idx=target_wall_idx,
            col_target=col_target,
            pos=pos_container[current_agent_idx],
            v=v_container[current_agent_idx],
            r=self.agent_list[current_agent_idx],
            t=col_t,
        )
        pos_container[current_agent_idx] = col_pos
        v_container[current_agent_idx] = col_v

        pos_container, v_container = self.update_other(
        ...
        if remaining_t <= 1e-14:
            if col_target == "wall":
                ...
                wall_endpoint = getattr(
                    self.map["objects"][target_wall_idx], col_target
                )
                connected_wall = self.point2wall[tuple(wall_endpoint)]
                for idx in connected_wall:
                    self._add_wall_ignore(
                        "wall", current_agent_idx, idx, ignore_wall_list, if_global=True
                    )
            else:
                self._add_wall_ignore(
                    col_target, current_agent_idx, target_wall_idx, None, if_global=True
                )  # collision of arc

        if col_target == "wall":
            ignore_wall_list = self._add_wall_ignore(
                "wall", current_agent_idx, target_wall_idx, ignore_wall_list
            )

            ignore_wall_list = self._add_wall_ignore(
                "l1", current_agent_idx, target_wall_idx, ignore_wall_list
            )
            ignore_wall_list = self._add_wall_ignore(
                "l2", current_agent_idx, target_wall_idx, ignore_wall_list
            )
        elif col_target == "l2" or col_target == "l1":
            ignore_wall_list = self._add_wall_ignore(
                col_target, current_agent_idx, target_wall_idx, ignore_wall_list
            )
            wall_endpoint = getattr(self.map["objects"][target_wall_idx], col_target)
            connected_wall = self.point2wall[tuple(wall_endpoint)]
            for idx in connected_wall:
                ignore_wall_list = self._add_wall_ignore(
                    "wall", current_agent_idx, idx, ignore_wall_list
                )
        else:
            ignore_wall_list = self._add_wall_ignore(
                col_target, current_agent_idx, target_wall_idx, ignore_wall_list
    ...
    def handle_circle(
        ...
        pos_col1, v1_col, pos_col2, v2_col = self.CCD_circle_collision(
            pos_container[current_circle_idx],
            pos_container[target_circle_idx],
            v_container[current_circle_idx],
            v_container[target_circle_idx],
            self.agent_list[current_circle_idx].r,
            self.agent_list[target_circle_idx].r,
            self.agent_list[current_circle_idx].mass,
            self.agent_list[target_circle_idx].mass,
            return_t=False,
        )
        ...
        pos_container, v_container = self.update_other(
            pos_container, v_container, col_t, [current_circle_idx, target_circle_idx]
        )
        remaining_t -= col_t
        if remaining_t <= 1e-14:
            self.global_circle_ignore.append(
                [current_circle_idx, target_circle_idx, 0.0]
            )  # adding instead of defining
    ...
    def stepPhysics(self, actions_list, step=None):
        ...
        while True:
            if self.print_log:
                ...
                print(
                    "The pos = {}, the v = {}".format(
                        temp_pos_container, temp_v_container
                    )
                )
            ...
            (
                ...
                target_wall_idx,
                current_agent_idx,
            ) = self.bounceable_wall_collision_time(
                temp_pos_container, temp_v_container, remaining_t, ignore_wall
            )  # collision detection with walls

            (
                earliest_circle_col_t,
                collision_circle_target,
                current_circle_idx,
                target_circle_idx,
            ) = self.circle_collision_time(
                temp_pos_container, temp_v_container, remaining_t, ignore_circle
            )

            if self.print_log:
                print(
                    "Wall t = {}, collide = {}, agent_idx = {}, wall_idx = {}".format(
                        earliest_wall_col_t,
                        collision_wall_target,
                        current_agent_idx,
                        target_wall_idx,
                ...
                print(
                    "Circle t = {}, collide = {}, agent_idx = {}, target_idx = {}".format(
                        earliest_circle_col_t,
                        collision_circle_target,
                        current_circle_idx,
                        target_circle_idx,
            ...
            if collision_wall_target is not None and collision_circle_target is None:
                ...
                if self.print_log:
                    print("NO COLLISION!")
                temp_pos_container, temp_v_container = self.update_all(
                    temp_pos_container, temp_v_container, remaining_t, temp_a_container
                )
        ...
        for agent_idx in range(self.agent_num):
            ...
            for object_idx in range(len(self.map["objects"])):
                ...
                if not object.can_pass():
                    ...
                    if object.color == "red" and object.check_cross(
                        self.agent_pos[agent_idx], agent.r
                    ):
                        agent.color = "red"
                        agent.finished = (
                            True  # when agent has crossed the finished line
                        )
    ...
    def cross_detect(self, previous_pos, new_pos):
        ...
        for agent_idx in range(self.agent_num):
            ...
            if (
                point2line(l1, l2, agent_new_pos) <= self.agent_list[agent_idx].r
            ) and final.check_on_line(closest_point(l1, l2, agent_new_pos)):
    ...
    def get_obs(self):
        ...
        for agent_idx, agent in enumerate(self.agent_list):
            ...
            visibility = self.agent_list[agent_idx].visibility  # view distance
            v_clear = self.agent_list[agent_idx].visibility_clear
            obs_size = int(visibility / v_clear)  # number of pixels
            view_back = visibility * self.VIEW_BACK  # distance of viewing back
            ...
            ViewCenter_x = (
                agent_current_boundary[0][0] + agent_current_boundary[2][0]
            ) / 2
            ViewCenter_y = (
                agent_current_boundary[0][1] + agent_current_boundary[2][1]
            ) / 2
            ViewCenter = [ViewCenter_x, ViewCenter_y]
            View_R = visibility * math.sqrt(2) / 2

            line_considered = []
            arc_considered = []
            agents_considered = []
            agents_considered_index = []

            for index_m, map_item in enumerate(
                self.map["objects"]
            ):  # select potentially touchable lines
                if (map_item.type == "wall") or (map_item.type == "cross"):
                    closest_dist = distance_to_line(
                        map_item.init_pos[0], map_item.init_pos[1], ViewCenter
                    )
                    if closest_dist <= View_R:
                        line_considered.append(map_item)
                elif map_item.type == "arc":
                    arc_center = map_item.center
                    # map_item.cur_pos = list()
                    center_dist_square = (ViewCenter_x - arc_center[0]) ** 2 + (
                        ViewCenter_y - arc_center[1]
                    ) ** 2
                    if center_dist_square <= (View_R + map_item.R) ** 2:
                        arc_considered.append(map_item)

            agent_self = self.agent_list[agent_idx]
            for aid, a_other in enumerate(
                self.agent_list
            ):  # select potentially touchable agents
                if aid == agent_idx:
                    continue

                target_agent = self.agent_list[aid]
                target_agent_pos = self.agent_pos[aid]
                center_dist_square = (ViewCenter_x - target_agent_pos[0]) ** 2 + (
                    ViewCenter_y - target_agent_pos[1]
                ) ** 2
                if center_dist_square <= (View_R + target_agent.r) ** 2:
                    agents_considered.append(a_other)
                    agents_considered_index.append(aid)

            obs_array = np.zeros((obs_size, obs_size))
            ################################ plot the line first ########################################
            for line in line_considered:
                assert line.type == "wall" or line.type == "cross"
                line_pos = line.init_pos
                line_rotate_pos = []
                for end_point in line_pos:
                    line_rotate_pos.append(
                        point_rotate([agent_x, agent_y], end_point, theta)
                    )

                # compute intersection point between line and the view boundary
                intersect_p = []
                rotate_boundary = [
                ...
                for bound in rotate_boundary:
                    _intersect_p = line_intersect(
                        line1=bound, line2=line_rotate_pos, return_p=True
                    )
                    if _intersect_p:
                        intersect_p.append(tuple(_intersect_p))
                intersect_p = list(set(intersect_p))

                draw_line = []
                if len(intersect_p) == 0:
                    point_1_in_view = (
                        0 < line_rotate_pos[0][0] + view_back < visibility
                        and abs(line_rotate_pos[0][1]) < visibility / 2
                    )
                    point_2_in_view = (
                        0 < line_rotate_pos[1][0] + view_back < visibility
                        and abs(line_rotate_pos[1][1]) < visibility / 2
                    )
                    if point_1_in_view and point_2_in_view:
                        draw_line.append(line_rotate_pos[0])
                        draw_line.append(line_rotate_pos[1])
                ...
                obs_array = DDA_line(
                    ...
                    visibility,
                    v_clear,
                    value=COLOR_TO_IDX[line.color],
                    view_back=view_back,
                )

            ############################# plot agent itself #####################################
            if self.VIEW_ITSELF:
                view_back_pixel = view_back / v_clear
                mid_j = (obs_size - 1) / 2
                mid_i = obs_size - view_back_pixel
                agent_r_pixel = self.agent_list[agent_idx].r / v_clear
                j_range = range(
                    int(np.floor(mid_j - agent_r_pixel)),
                    int(np.ceil(mid_j + agent_r_pixel)) + 1,
                )
                i_range = range(
                    int(np.floor(mid_i - agent_r_pixel)),
                    min(int(np.ceil(mid_i + agent_r_pixel)), obs_size - 1) + 1,
                )
                for i in i_range:
                    x = visibility - v_clear * i - view_back
                    for j in j_range:
                        y = visibility / 2 - v_clear * j
                        point = (x, y)
                        self_center = [0, 0]
                        dis_to_itself = math.sqrt(
                            (point[0] - self_center[0]) ** 2
                            + (point[1] - self_center[1]) ** 2
                        )
                        if dis_to_itself <= self.agent_list[agent_idx].r + v_clear / 2:
                            obs_array[i][j] = COLOR_TO_IDX[
                                self.agent_list[agent_idx].color
                            ]

            ################################## plot other agents ##################################
            for a_idx, plot_agent in enumerate(agents_considered):
                assert plot_agent.type == "agent" or plot_agent.type == "ball"
                plot_agent_pos = self.agent_pos[agents_considered_index[a_idx]]

                # the axis of plot agent pos was the default pygame axises,
                # now the axis has rotated (the axis of the view array),
                # so we need to computed the transformmed coord of the plot agent on the new axis (of view array)

                # first compute target agent relative coord to the view center at pygame default axis
                _x = plot_agent_pos[0] - ViewCenter_x
                _y = plot_agent_pos[1] - ViewCenter_y
                # flip the y axis for easy understanding
                _y = -_y
                # now the axis has rotate clockwise theta, the coord of target agent has transformed
                theta_prime = -theta_copy * math.pi / 180  # theta has opponent sign
                _x_prime = math.cos(theta_prime) * _x + math.sin(theta_prime) * _y
                _y_prime = -math.sin(theta_prime) * _x + math.cos(theta_prime) * _y
                _y_prime = -_y_prime
                # now shift the new coord to match the obs array
                x_prime = _x_prime + visibility / 2
                y_prime = _y_prime + visibility / 2
                target_r = plot_agent.r

                addon = 1
                x_range_min = max(
                    0, int(np.floor(obs_size - (x_prime + target_r) / v_clear)) - addon
                )
                x_range_max = min(
                    obs_size,
                    int(np.ceil(obs_size - (x_prime - target_r) / v_clear)) + addon,
                )
                y_range_min = max(
                    0, int(np.floor((y_prime - target_r) / v_clear)) - addon
                )
                y_range_max = min(
                    obs_size, int(np.ceil((y_prime + target_r) / v_clear)) + addon
                )
                for i in range(x_range_min, x_range_max):
                    x = visibility - v_clear * i
                    for j in range(y_range_min, y_range_max):
                        y = v_clear * j
                        dist = math.sqrt((x - x_prime) ** 2 + (y - y_prime) ** 2)
                        if dist <= target_r + v_clear / 2:
                            obs_array[i][j] = COLOR_TO_IDX[plot_agent.color]
                """             x_prime
                                ^
                                |
                                |----------
            ...
            ############################ plot arc #################################
            for arc in arc_considered:
                arc_r = arc.R
                arc_center = arc.center
                # s_angle, e_angle = arc.start_radian, arc.end_radian
                # s_point = [arc_r*math.cos(s_angle)+arc_center[0], arc_center[1]-arc_r*math.sin(s_angle)]
                # e_point = [arc_r*math.cos(e_angle)+arc_center[0], arc_center[1]-arc_r*math.sin(e_angle)]
                _x = arc_center[0] - ViewCenter_x
                _y = arc_center[1] - ViewCenter_y
                _y = -_y
                theta_prime = -theta_copy  # theta has opponent sign
                _x_prime, _y_prime = rotate2(_x, _y, theta_prime)
                _y_prime = -_y_prime
                x_prime = _x_prime + visibility / 2
                y_prime = _y_prime + visibility / 2

                addon = 1  # enlarge the search space to avoid numerical error during rounding up
                x_range_min = max(
                    0, int(np.ceil(obs_size - (x_prime + arc_r) / v_clear)) - addon
                )
                x_range_max = min(
                    obs_size,
                    int(np.ceil(obs_size - (x_prime - arc_r) / v_clear)) + addon,
                )
                y_range_min = max(0, int(np.floor((y_prime - arc_r) / v_clear)) - addon)
                y_range_max = min(
                    obs_size, int(np.ceil((y_prime + arc_r) / v_clear)) + addon
                )
                ...
                for i in range(x_range_min, x_range_max):
                    ...
                    for j in range(y_range_min, y_range_max):
                        ...
                        original_point = rotate2(
                            x - visibility / 2, -(y - visibility / 2), -theta_prime
                        )
                        # rotate the transformed coord back to the pygame axises, first shift to the view center axis, flip the y axis
                        #           and rotate2 with -theta_prime
                        original_point = [
                            original_point[0] + ViewCenter_x,
                            ViewCenter_y - original_point[1],
                        ]
                        # flip the y axis back and shift to pygame axises
                        p2center = [
                            original_point[0] - arc_center[0],
                            original_point[1] - arc_center[1],
                        ]
                        angle = math.atan2(
                            -p2center[1], p2center[0]
                        )  # negative y since the pygame axis y axis is reversed

                        if get_obs_check_radian(
                            arc.start_radian, arc.end_radian, angle
                        ):
                            dist = math.sqrt((x - x_prime) ** 2 + (y - y_prime) ** 2)
                            if (dist <= arc_r + v_clear / 2) and (
                                dist >= arc_r - v_clear / 2
                            ):
                                obs_array[i][j] = COLOR_TO_IDX[arc.color]
    ...
    def get_obs_deprecated(self):
        ...
        for agent_idx, agent in enumerate(self.agent_list):
            ...
            visibility = self.agent_list[agent_idx].visibility  # view distance
            v_clear = self.agent_list[
                agent_idx
            ].visibility_clear  # distance for each pixel
            ...
            view_center_x = agent_x + (visibility / 2 - view_back / 2) * math.cos(
                theta * math.pi / 180
            )  # start from agent x,y
            view_center_y = agent_y + (visibility / 2 - view_back / 2) * math.sin(
                theta * math.pi / 180
            )
            ...
            for index_m, item in enumerate(self.map["objects"]):
                if (item.type == "wall") or (item.type == "cross"):
                    closest_dist = distance_to_line(
                        item.init_pos[0], item.init_pos[1], view_center
                    )
                    if closest_dist <= view_R:
            ...
            for c in self.map["objects"]:
                if (c.type == "wall") or (c.type == "cross"):
                    ...
                    distance = distance_2points(
                        [c.cur_pos[0][0] - vec_oc_[0], c.cur_pos[0][1] - vec_oc_[1]]
                    )
                    ...
                    raise ValueError(
                        "No such object type- {}. Please check scenario.json".format(
                            c.type
                        )
                    )
            ...
            for a_i, a_other in enumerate(self.agent_list):
                if a_i == agent_idx:
                    ...
                    threshold = (
                        self.agent_list[agent_idx].visibility * 1.415 / 2 + a_other.r
                    )  # 默认视线为正方形
            ...
            for obj in line_consider:
                if obj.type == "wall" or obj.type == "cross":
                    ...
                    for line in rotate_boundary:
                        _intersect_p = line_intersect(
                            line1=line, line2=obj.rotate_pos, return_p=True
                        )
                        if _intersect_p:
                    ...
                    if len(intersect_p) == 0:
                        point_1_in_view = (
                            0 < obj.rotate_pos[0][0] + view_back < visibility
                            and abs(obj.rotate_pos[0][1]) < visibility / 2
                        )
                        point_2_in_view = (
                            0 < obj.rotate_pos[1][0] + view_back < visibility
                            and abs(obj.rotate_pos[1][1]) < visibility / 2
                        )
            ...
            # identify self-view square location
            if self.VIEW_ITSELF:
                view_back_pixel = view_back / v_clear
                mid_j = (obs_size - 1) / 2
                mid_i = obs_size - view_back_pixel
                agent_r_pixel = self.agent_list[agent_idx].r / v_clear
                j_range = range(
                    int(np.round(mid_j - agent_r_pixel)),
                    int(np.round(mid_j + agent_r_pixel)) + 1,
                )
                i_range = range(
                    int(np.round(mid_i - agent_r_pixel)),
                    min(int(np.round(mid_i + agent_r_pixel)), obs_size - 1) + 1,
                )
                for i in i_range:
                    x = visibility - v_clear * i - view_back
                    for j in j_range:
                        ...
                        if dis_to_itself <= self.agent_list[agent_idx].r:
                            obs_map[i][j] = COLOR_TO_IDX[
                                self.agent_list[agent_idx].color
                            ]

            for component in list(
                reversed(map_objects)
            ):  # reverse to consider agent first, then wall
                for i in range(obs_size):
                    if self.VIEW_ITSELF:
                        x = visibility - v_clear * i - view_back
                    else:
                        x = agent.r + visibility - v_clear * i
                    ...
                    for j in range(obs_size):
                        ...
                        if obs_map[i][j] > 0 and (
                            component.type != "agent" and component.type != "ball"
                        ):  # when there is already object on this pixel
                            continue
                        else:
                            if (component.type == "wall") or (
                                component.type == "cross"
                            ):  # no need to plot the line naively anymore
                                raise NotImplementedError
                                ...
                                vec_bc_ = (
                                    x - agent_self.to_another_agent_rotated[idx][0],
                                ...
                                radius = component.R
                                x_2center, y_2center = (
                                    x - component.cur_pos_rotated[0][0],
                                    y - component.cur_pos_rotated[0][1],
                                )
                                theta_pixel = theta
                                pos_x_2center, pos_y_2center = rotate2(
                                    x_2center, y_2center, theta_pixel
                                )
                                angle = math.atan2(pos_y_2center, pos_x_2center)
                                start_radian, end_radian = (
                                    component.start_radian,
                                    component.end_radian,
                                )
                                if get_obs_check_radian(
                                    start_radian, end_radian, angle
                                ):
                                    ...
                                    if (distance <= radius + v_clear / 2) and (
            ...
            if self.print_log2:
                print(
                    "agent {} get obs duration {}".format(
                        agent_idx, time.time() - time_stamp
                    )
                )
        self.obs_list = obs_list
    ...
    def change_inner_state(self):
        for agent_idx in range(self.agent_num):
            ...
            if (
                self.agent_list[agent_idx].energy < 0
            ):  # once fatigue, the agent died and lose control
                remaining_energy = -1
            else:
                ...
                force = self.agent_list[agent_idx].mass * math.sqrt(
                    self.agent_accel[agent_idx][0] ** 2
                    + self.agent_accel[agent_idx][1] ** 2
                )
                v = math.sqrt(
                    self.agent_v[agent_idx][0] ** 2 + self.agent_v[agent_idx][1] ** 2
                )
                consume_rate = force * v / 50
                consume_rate -= self.energy_recover_rate

                remaining_energy = (
                    self.agent_list[agent_idx].energy - consume_rate * self.tau
                )
                # force = force_idx
    ...
    def render(self, info=None):
        ...
        if self.draw_obs:
            self.viewer.draw_obs(self.obs_boundary, self.agent_list)
            self.viewer.draw_view(
                self.obs_list, self.agent_list, leftmost_x=500, upmost_y=10
            )
